...
// Input classes
// class 0: small operands
// class 1: full/random operands
void prepare_inputs(dudect_config_t *c, u8 *input_data, u8 *classes)
{
    for(size_t i = 0; i < c->number_measurements; i++)
    {
        struct mult_input *input = (struct mult_input *)(input_data + i * c->chunk_size);
        ...
        {
             // small values
            input->a = (u16)(dudect_xorshift64() % ML_KEM_Q);
            input->b = (u16)(dudect_xorshift64() % ML_KEM_Q);
        }else
        {
             // large/random values
            input->a = (u16)dudect_xorshift64();
            input->b = (u16)dudect_xorshift64();
        }
    }
}
...
{
    struct mult_input *input = (struct mult_input *)data;

    u16 r = ml_kem_multipl_mod(input->a, input->b);
...
int main(void)
{
    ...
    config.chunk_size = sizeof(struct mult_input);
