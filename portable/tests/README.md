...
This test suite validates the portable ML-KEM core implementation located in `portable/src`.

All tests are executed through the Linux userspace port because it provides the most convenient development and testing environment. 
Since the userspace port does not modify the portable cryptographic core, the test results remain representative of the implementation itself.

Environment-specific functionality introduced by individual ports (such as operating system integration, memory allocation, synchronization primitives,
or platform-specific interfaces) is validated separately within the corresponding port directories under `portable/ports/`.
