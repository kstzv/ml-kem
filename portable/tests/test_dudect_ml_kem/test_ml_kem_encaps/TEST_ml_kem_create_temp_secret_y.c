...
#define INPUT_SIZE ML_KEM_SEED_BYTES // 32 bytes for seed
...
{
    static u8 raw_bytes[ML_KEM_VECTOR_XOF_BYTES_L34];
    static u16 y[ML_KEM_N * ML_KEM_1024];

    memset(&kem_ctx, 0, sizeof(kem_ctx));

    kem_ctx.k = ML_KEM_1024; 
    kem_ctx.raw_bytes = raw_bytes;
    kem_ctx.y = y;
}
...
{
    memset(kem_ctx.raw_bytes, 0, ML_KEM_VECTOR_XOF_BYTES_L34);
    memset(kem_ctx.y, 0, sizeof(u16) * ML_KEM_N * kem_ctx.k);

    // Function for test
    ml_kem_create_temp_secret_y(&kem_ctx, data);

    // Protection against optimization
    dudect_sink ^= (u8)(kem_ctx.y[0] & 0xFF);
