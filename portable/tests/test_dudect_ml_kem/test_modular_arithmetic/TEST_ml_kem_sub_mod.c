...
void prepare_inputs(dudect_config_t *c, u8 *input_data, u8 *classes)
{
    ...
    {
        struct sub_input *input = (struct sub_input *)(input_data + i * c->chunk_size);
        ...
        if(classes[i] == 0)
        {
            ...
            input->a = (u16)(input->b + (dudect_xorshift64() % (ML_KEM_Q - input->b)));
        }else
        {
		// a < b
		input->a = (u16)(dudect_xorshift64() % (ML_KEM_Q - 1));

		input->b = (u16)(input->a + 1 + (dudect_xorshift64() % (ML_KEM_Q - input->a - 1)));
		}
    }
}
...
{
    struct sub_input *input = (struct sub_input *)data;

    u16 r = ml_kem_sub_mod(input->a, input->b);
...
int main(void)
{
    ...
    config.chunk_size = sizeof(struct sub_input);
