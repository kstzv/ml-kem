...
static void ml_kem_unpack_pk_t_u16(struct ml_kem_encaps_ctx *ctx);
...
struct ml_kem_encaps_ctx *ml_kem_alloc_encaps(u8 *public_key_msg, enum ml_kem_k k)
{
	...
	if(ML_KEM_512 > k || ML_KEM_1024 < k) { return ERR_PTR(-EINVAL); }
	
	// Allocate main context structure
	struct ml_kem_encaps_ctx *ctx;
	ctx = kzalloc(sizeof(struct ml_kem_encaps_ctx), GFP_KERNEL);
	if(!ctx) { return ERR_PTR(-ENOMEM); }
	memzero_explicit(ctx, sizeof(struct ml_kem_encaps_ctx));
	...
	ctx->u = kzalloc(sizeof(u16) * ML_KEM_N * (3 * ctx->k + 2), GFP_KERNEL);
	if(!ctx->u) { goto first_error_exit; }
	memzero_explicit(ctx->u, sizeof(u16) * ML_KEM_N * (3 * ctx->k + 2));
	...
	ctx->ciphertext = kzalloc(ctx->ciphertext_len, GFP_KERNEL);
	if(!ctx->ciphertext) { goto second_error_exit; }
	memzero_explicit(ctx->ciphertext, ctx->ciphertext_len);
	
	// Allocate buffer for serialized public key + extra space
	// For XOF-generated raw bytes (used during matrix expansion)
	ctx->public_key_msg = kzalloc(ctx->public_key_msg_len + ML_KEM_MATRIX_XOF_BYTES, GFP_KERNEL);
	if(!ctx->public_key_msg) { goto third_error_exit; }
	memzero_explicit(ctx->public_key_msg, ctx->public_key_msg_len + ML_KEM_MATRIX_XOF_BYTES);
	
	// Assign raw_bytes buffer (placed right after public_key_msg)
...
void ml_kem_wipe_encaps(struct ml_kem_encaps_ctx *ctx)
{
	...
	if(ctx->ciphertext) { memzero_explicit(ctx->ciphertext, ctx->ciphertext_len); }
	...
	{
		memzero_explicit(ctx->raw_bytes, ML_KEM_MATRIX_XOF_BYTES);
	}
	
	// Zeroize main polynomial buffer containing:
	// u, v, unpacked public key, temporary secret y, and working polynomial
	if(ctx->u) { memzero_explicit(ctx->u, sizeof(u16) * ML_KEM_N * (3 * ctx->k + 2)); }
	
	// Zeroize internal seed and shared secret buffers
	memzero_explicit(ctx->seed_rho, ML_KEM_SEED_BYTES);
	memzero_explicit(ctx->seed_m, ML_KEM_SEED_BYTES);
	memzero_explicit(ctx->K_bar, ML_KEM_SEED_BYTES);
}
...
void ml_kem_destroy_encaps(struct ml_kem_encaps_ctx *ctx)
{
	...
	if(ctx->ciphertext) { kfree(ctx->ciphertext); }
...
void ml_kem_encapsulation(struct ml_kem_encaps_ctx *ctx, u8 *msg, u8 *hash_pk)
{
	...
	// Reset internal buffers before computation
	memzero_explicit(ctx->u, sizeof(u16) * ML_KEM_N * ctx->k);
	memzero_explicit(ctx->v, sizeof(u16) * ML_KEM_N);
	memzero_explicit(ctx->ciphertext, ctx->ciphertext_len);
	...
	{   // Client-side path:
		...
		get_random_bytes(first_seed_encaps, ML_KEM_SEED_BYTES);
    ...
    memzero_explicit(first_seed_encaps, sizeof(first_seed_encaps));
    ...
    memzero_explicit(result_seeds, sizeof(result_seeds));
}
...
static void ml_kem_create_temp_secret_y(struct ml_kem_encaps_ctx *ctx, u8 *seed)
{
	...
	for(u8 i = 0; i < ctx->k; i++)
	{
		...
		// Clear temporary buffer with raw bytes
		memzero_explicit(ctx->raw_bytes, number_bytes);
	}
}
...
static int ml_kem_create_u(struct ml_kem_encaps_ctx *ctx, u8 *seed)
{
    ...
    for(size_t i = 0; i < ctx->k; i++)
    {
		for(size_t j = 0; j < ctx->k; j++)
		{
			...
			if(ret < 0) 
			{
				 memzero_explicit(ctx->poly, sizeof(u16) * ML_KEM_N);
...
static void ml_kem_create_v(struct ml_kem_encaps_ctx *ctx, u8 *seed)
{
	...
	while(msg_bytes_counter < 32)
	{
		for(int i = 0; i < 8; i++)
		{
			temp = (u16)(local_msg[msg_bytes_counter] & 0x1); 		// Extract least significant bit
			ctx->poly[poly_counter] = temp * (u16)ML_KEM_MSG_COEFF; // Map bit to coefficient value
			local_msg[msg_bytes_counter] >>= 1; 					// Shift to process next bit
			poly_counter++;
		}
		msg_bytes_counter++; 
	}
	...
	memzero_explicit(ctx->poly, sizeof(u16) * ML_KEM_N);
	memzero_explicit(local_msg, sizeof(local_msg));
	memzero_explicit(local_seed_noise, sizeof(local_seed_noise));
}
...
static void ml_kem_compress_ciphertext(struct ml_kem_encaps_ctx *ctx)
{
	...
	size_t pos = 0;
	size_t step_bytes = d_u * ML_KEM_N / 8; // Number of bytes per compressed polynomial of u
