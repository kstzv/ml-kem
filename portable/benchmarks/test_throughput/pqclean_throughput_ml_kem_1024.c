...
uint8_t shared_secrets[NUMBER_ITERATIONS][PQCLEAN_MLKEM1024_CLEAN_CRYPTO_BYTES];
uint8_t ciphertexts[NUMBER_ITERATIONS][PQCLEAN_MLKEM1024_CLEAN_CRYPTO_CIPHERTEXTBYTES];
...
void *worker(void *arg)
{
    ...
    uint8_t decaps_shared_secret[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_BYTES];
    ...
    while(1)
    {
        ...
        int ret = PQCLEAN_MLKEM1024_CLEAN_crypto_kem_dec(decaps_shared_secret, ciphertexts[idx], args->secret);
        if(ret != 0)
        {
        ...
        ret = memcmp(decaps_shared_secret, shared_secrets[idx], PQCLEAN_MLKEM1024_CLEAN_CRYPTO_BYTES);
        if(ret != 0)
        {
...
int main()
{
	...
	uint8_t secret[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_SECRETKEYBYTES];
	...
	// Create keys
	ret = PQCLEAN_MLKEM1024_CLEAN_crypto_kem_keypair(pk, secret);
	if(ret != 0) { printf("Error in create keys\n"); return -1; }
	
	// Create ciphertexts
	for(size_t i = 0; i < NUMBER_ITERATIONS; i++)
	{
		ret = PQCLEAN_MLKEM1024_CLEAN_crypto_kem_enc(ciphertexts[i], shared_secrets[i], pk);
