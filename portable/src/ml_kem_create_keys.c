...
int ml_kem_create_ctx_struct(struct ml_kem_temp *temp, struct ml_kem_ctx *ctx, ml_kem_entropy_fn entropy)
{
	...
	{ return -ML_KEM_EINVAL; }
	
	// Validate persistent context and required buffers
	if(!ctx || !ctx->secret || !ctx->public_key_msg) { return -ML_KEM_EINVAL; }
	
	// Validate ML-KEM parameters (defensive checks)
	if(temp->eta != ML_KEM_CBD_ETA_3 && temp->eta != ML_KEM_CBD_ETA_2) { return -ML_KEM_EINVAL; }
	else if(temp->k != ML_KEM_512 && temp->k != ML_KEM_768 && temp->k != ML_KEM_1024) { return -ML_KEM_EINVAL; }
	else if(temp->number_raw_bytes_vectors != ML_KEM_VECTOR_XOF_BYTES_L2 && temp->number_raw_bytes_vectors != ML_KEM_VECTOR_XOF_BYTES_L34) { return -ML_KEM_EINVAL; }
	else if(ctx->k != temp->k) { return -ML_KEM_EINVAL; }
...
struct ml_kem_temp *ml_kem_temp_alloc(enum ml_kem_k level)
{
	...
	temp = ml_kem_alloc(sizeof(struct ml_kem_temp));
	if(!temp) { return NULL; }
	...
	temp->poly = ml_kem_alloc(sizeof(u16) * ML_KEM_N * (2 * temp->k + 1));
	...
	temp->raw_bytes = ml_kem_alloc(ML_KEM_MATRIX_XOF_BYTES);
...
struct ml_kem_ctx *ml_kem_ctx_alloc(enum ml_kem_k level)
{
	...
	ctx = ml_kem_alloc(sizeof(struct ml_kem_ctx));
	if(!ctx) { return NULL; }
	...
	ctx->secret = ml_kem_alloc(sizeof(u16) * ctx->k * ML_KEM_N);
	...
	ctx->public_key_msg = ml_kem_alloc(ctx->public_key_msg_len);
...
void ml_kem_destroy_ctx_struct(struct ml_kem_ctx *ctx)
{
	...
	{
		ml_kem_free(ctx->public_key_msg);
...
STATIC int ml_kem_generation_entropy(struct ml_kem_temp *temp, ml_kem_entropy_fn entropy)
{
    ...
    if(entropy == NULL)
    {
		if(ml_kem_entropy(first_seed, ML_KEM_SEED_BYTES) != 0) { return -ML_KEM_SYSTEM_ENTROPY_FAILED; }
	}else
	{
		if(entropy(first_seed, ML_KEM_SEED_BYTES) != 0) { return -ML_KEM_CALLBACK_ENTROPY_FAILED; }
	}
...
STATIC int ml_kem_finally_create_z_and_h_pk(struct ml_kem_ctx *ctx, ml_kem_entropy_fn entropy)
{
	...
	if(entropy == NULL)
    {
		if(ml_kem_entropy(ctx->z, ML_KEM_SEED_BYTES) != 0) { return -ML_KEM_SYSTEM_ENTROPY_FAILED; }
	}else
	{
		if(entropy(ctx->z, ML_KEM_SEED_BYTES) != 0) { return -ML_KEM_CALLBACK_ENTROPY_FAILED; }
	}
