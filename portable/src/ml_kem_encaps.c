...
struct ml_kem_encaps_ctx *ml_kem_alloc_encaps(u8 *public_key_msg, enum ml_kem_k k)
{
	...
	ctx = ml_kem_alloc(sizeof(struct ml_kem_encaps_ctx));
	if (!ctx) { return NULL; }
	...
	ctx->u = ml_kem_alloc(sizeof(u16) * ML_KEM_N * (3 * ctx->k + 2));
	...
	ctx->ciphertext = ml_kem_alloc(ctx->ciphertext_len);
	...
	ctx->public_key_msg = ml_kem_alloc(ctx->public_key_msg_len + ML_KEM_MATRIX_XOF_BYTES);
...
void ml_kem_destroy_encaps(struct ml_kem_encaps_ctx *ctx)
{
	...
	if(ctx->ciphertext) { ml_kem_free(ctx->ciphertext); }
...
void ml_kem_encapsulation(struct ml_kem_encaps_ctx *ctx, u8 *msg, u8 *hash_pk, ml_kem_entropy_fn entropy)
{
	...
	else
	{
		...
		if(entropy == NULL)
		{
			if(ml_kem_entropy(first_seed_encaps, ML_KEM_SEED_BYTES) != 0) { ctx->err = -ML_KEM_SYSTEM_ENTROPY_FAILED; return; }
		}else
		{
			if(entropy(first_seed_encaps, ML_KEM_SEED_BYTES) != 0) { ctx->err = -ML_KEM_CALLBACK_ENTROPY_FAILED; return; }
		}
