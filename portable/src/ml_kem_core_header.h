...
extern void *ml_kem_alloc(size_t size_alloc);
extern void ml_kem_free(void *ptr);
...
extern bool ml_kem_try_acquire_slot(ml_kem_atomic_t *slot);
extern void ml_kem_release_slot(ml_kem_atomic_t *slot);
extern void ml_kem_atomic_init_slot(ml_kem_atomic_t *slot);

struct ml_kem_pool_decaps_ctx *err_create_keys(const int err);
u8 *err_encaps(const int err);
int err_decaps(const int err);
int err_get_pk(const int err);
...
struct ml_kem_decaps_ctx {
	...
	ml_kem_atomic_t is_free;                            // Atomic flag for slot availability
};
