...
void ml_kem_wipe_decrypt(struct ml_kem_decrypt_ctx *ctx_decry)
{
	...
	if(ctx_decry->u) { ml_kem_memzero(ctx_decry->u, sizeof(u16) * (ctx_decry->ctx->k + 2) * ML_KEM_N); }
	
	// Zeroize stored ciphertext
	if(ctx_decry->ciphertext) { ml_kem_memzero(ctx_decry->ciphertext, ctx_decry->ciphertext_len); }
	
	// Zeroize recovered message seed (m)
	ml_kem_memzero(ctx_decry->m, 32);
}
...
static void ml_kem_decrypt_get_poly(struct ml_kem_decrypt_ctx *ctx_decry)
{
	...
	ml_kem_memzero(ctx_decry->poly, sizeof(u16) * ML_KEM_N);
