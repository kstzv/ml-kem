...
static void ml_kem_decaps_ct_select_ss(struct ml_kem_pool_decaps_ctx *pool, u8 *ciphertext, size_t len_ciphertext, u8 *result, size_t curr_slot)
{
	...
	{
		value |= ciphertext[i] ^ pool->ml_kem_pool[curr_slot].encaps_ctx->ciphertext[i];
	}
	
	// value: 0 if match, !=0 if mismatch
	u8 fail = (value != 0);      // 0 or 1
	u8 mask = (u8)-(int)fail;    // 0x00 or 0xFF
	u8 inv  = (u8)~mask;         // 0xFF or 0x00
