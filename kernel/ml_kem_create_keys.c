...
int ml_kem_create_ctx_struct(struct ml_kem_temp *temp, struct ml_kem_ctx *ctx)
{
	...
	// Step 2: Generate secret key vector s (CBD sampling in NTT domain)// Створення закритого ключа
...
struct ml_kem_temp *ml_kem_temp_alloc(enum ml_kem_k level)
{
	...
	temp = kzalloc(sizeof(struct ml_kem_temp), GFP_KERNEL);
	if(!temp) { return ERR_PTR(-ENOMEM); }
	memzero_explicit(temp, sizeof(struct ml_kem_temp));
	...
	temp->poly = kzalloc(sizeof(u16) * ML_KEM_N * (2 * temp->k + 1), GFP_KERNEL);
	if(!temp->poly) { goto first_error_exit; }
	memzero_explicit(temp->poly, sizeof(u16) * ML_KEM_N * (2 * temp->k + 1));
	...
	temp->raw_bytes = kzalloc(ML_KEM_MATRIX_XOF_BYTES, GFP_KERNEL);
	if(!temp->raw_bytes) { goto second_error_exit; }
	memzero_explicit(temp->raw_bytes, ML_KEM_MATRIX_XOF_BYTES);
...
struct ml_kem_ctx *ml_kem_ctx_alloc(enum ml_kem_k level)
{
	...
	ctx = kzalloc(sizeof(struct ml_kem_ctx), GFP_KERNEL);
	if(!ctx) { return ERR_PTR(-ENOMEM); }
	memzero_explicit(ctx, sizeof(struct ml_kem_ctx));
	...
	ctx->secret = kzalloc(sizeof(u16) * ctx->k * ML_KEM_N, GFP_KERNEL);
	if(!ctx->secret) { goto first_error_exit; }
	memzero_explicit(ctx->secret, sizeof(u16) * ctx->k * ML_KEM_N);
	...
	ctx->public_key_msg = kzalloc(ctx->public_key_msg_len, GFP_KERNEL);
	if(!ctx->public_key_msg) { goto second_error_exit; }
	memzero_explicit(ctx->public_key_msg, ctx->public_key_msg_len);
	
	// Все зроблено, повертаємо наш вказівник
	return ctx;
...
void ml_kem_destroy_temp_struct(struct ml_kem_temp *temp)
{
	...
	if(temp->poly)
	{
		memzero_explicit(temp->poly, sizeof(u16) * (temp->k * 2 + 1) * ML_KEM_N);
		kfree(temp->poly);
		temp->poly = NULL;
	...
	memzero_explicit(temp->seed_sigma, sizeof(temp->seed_sigma));
	memzero_explicit(temp->seed_rho, sizeof(temp->seed_rho));
...
void ml_kem_destroy_ctx_struct(struct ml_kem_ctx *ctx)
{
	...
	memzero_explicit(ctx->z, ML_KEM_SEED_BYTES);
    memzero_explicit(ctx->h_pk, ML_KEM_SEED_BYTES);
...
static int ml_kem_generation_entropy(struct ml_kem_temp *temp)
{
	...
	get_random_bytes(first_seed, ML_KEM_SEED_BYTES);
	
	first_seed[32] = (u8)temp->k; // Append ML-KEM parameter k (matrix/vector dimension)
	...
	memzero_explicit(first_seed, sizeof(first_seed));
	...
	memzero_explicit(seeds_rho_and_sigma, sizeof(seeds_rho_and_sigma));
...
static void ml_kem_create_vector_poly(struct ml_kem_temp *temp)
{
	...
	for(size_t i = 0; i < temp->k; i++)
	{
		...
		memzero_explicit(temp->raw_bytes, temp->number_raw_bytes_vectors);
	}
}
...
static int ml_kem_create_public_key(struct ml_kem_temp *temp)
{
	...
	for(size_t row = 0; row < temp->k; row++)
	{
		for(size_t col = 0; col < temp->k; col++)
		{
			...
			{ 
				memzero_explicit(temp->raw_bytes, ML_KEM_MATRIX_XOF_BYTES);
                memzero_explicit(local_seed_noise, sizeof(local_seed_noise));
                memzero_explicit(local_seed_matrix, sizeof(local_seed_matrix));
                memzero_explicit(temp->poly, sizeof(u16) * ML_KEM_N);
				return ret;
			}
...
static int ml_kem_finally_create_z_and_h_pk(struct ml_kem_ctx *ctx)
{
	...
	get_random_bytes(ctx->z, ML_KEM_SEED_BYTES);
...
void ml_kem_gen_polynomial_for_cbd(u16 *poly, const u8 *stream, const u8 eta)
{
	...
	if(eta == 2) // 64-bit package → 16 coefficients
	{
		one_package_bits = 64;
		one_package_bytes = 8;
	}else{      // 48-bit package → 8 coefficients
		one_package_bits = 48;
		one_package_bytes = 6;
	}
	...
	int conter_coeff = 0; 		// Output coefficient index
	int size_bytes_counter = 0; // Bit/byte tracking inside buffer
	int counter_bits = 0;
...
int ml_kem_gen_polynomial_for_matrix(u16 *poly, const u8 *stream, int suma_bytes)
{
        ...
        while (j < ML_KEM_N)
        {
			if(suma_bytes < 3 ) { return -EAGAIN; } 	     // Need at least 3 bytes to extract two candidates
            ...
            pos += 3;		// Advance stream position
            d1 = d2 = 0; 	// Reset temporary variables
            suma_bytes -= 3;// Track remaining bytes
		} 
	return suma_bytes;
}
